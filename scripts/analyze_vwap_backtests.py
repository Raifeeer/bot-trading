"""Analiza la matriz VWAP y compara contra DayBreakout por ventana y símbolo."""
...
BACKTEST = ROOT / "backtests/vwap_backtests_2026-08-19.csv"
TRADES = ROOT / "backtests/vwap_backtest_trades_2026-08-19.csv"
OUT = ROOT / "backtests"
...
def main() -> None:
    ...
    comparison = candidates.merge(
        baseline.drop(columns=["timeframe"]), on=["window"], how="left"
    )
    ...
    if not trades.empty:
        ...
        symbol_summary["return_pct_on_symbol_slice"] = (
            symbol_summary["pnl"] / (START_CAPITAL / 7.0) * 100.0
        )
        symbol_summary["win_rate_pct"] = (
            symbol_summary["wins"] / symbol_summary["trades"] * 100.0
        )
    else:
        symbol_summary = pd.DataFrame()

    comparison.to_csv(OUT / "vwap_backtest_comparison_2026-08-19.csv", index=False)
    summary.to_csv(OUT / "vwap_backtest_variant_summary_2026-08-19.csv", index=False)
    symbol_summary.to_csv(OUT / "vwap_backtest_symbol_summary_2026-08-19.csv", index=False)
    print("BASELINE")
    ...
    full = summary.merge(
        comparison[
            (comparison["window"] == "full_available") & comparison["comparison_valid"]
        ][
            [
                "variant",
                "timeframe",
    ...
    print("\nSYMBOL SUMMARY")
    print(symbol_summary.sort_values("pnl", ascending=False).head(30).to_string(index=False))
