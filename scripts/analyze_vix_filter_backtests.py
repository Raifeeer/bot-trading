"""Resumen reproducible de VIX filter versus baseline."""
...
def main() -> None:
    df = pd.read_csv(ROOT / "vix_filter_backtests_2026-08-18_results.csv")
    ...
    deltas.to_csv(ROOT / "vix_filter_backtests_2026-08-18_deltas.csv", index=False)
    ...
    summary.to_csv(ROOT / "vix_filter_backtests_2026-08-18_variant_summary.csv", index=False)
    robust = summary[(summary["windows_beating_return"] >= 3)
                     & (summary["windows_not_worse_drawdown"] >= 3)]
    robust.to_csv(ROOT / "vix_filter_backtests_2026-08-18_robust_candidates.csv", index=False)
