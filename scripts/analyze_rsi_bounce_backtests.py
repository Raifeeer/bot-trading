"""Analiza la matriz RSI bounce sobre SMA200 frente a DayBreakout."""
...
BACKTEST = ROOT / "backtests/rsi_bounce_backtests_2026-08-19.csv"
TRADES = ROOT / "backtests/rsi_bounce_backtest_trades_2026-08-19.csv"
OUT = ROOT / "backtests"
...
def main() -> None:
    ...
    comparison.to_csv(OUT / "rsi_bounce_backtest_comparison_2026-08-19.csv", index=False)
    summary.to_csv(OUT / "rsi_bounce_backtest_variant_summary_2026-08-19.csv", index=False)
    symbol_summary.to_csv(OUT / "rsi_bounce_backtest_symbol_summary_2026-08-19.csv", index=False)
    print("BASELINE")
    print(baseline.to_string(index=False))
    print("\nTOP SUMMARY")
    print(summary.head(50).to_string(index=False))
    print("\nFULL AVAILABLE")
    print(comparison[(comparison["window"] == "full_available") & comparison["comparison_valid"]].sort_values("delta_return_pp", ascending=False).head(50).to_string(index=False))
    print("\nSYMBOL SUMMARY")
    print(symbol_summary.sort_values("pnl", ascending=False).head(50).to_string(index=False))
