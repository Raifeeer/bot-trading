...
from scripts.run_breakout_20_55_backtests import scan_events
...
CANDIDATES = [
    {"name": "breakout_15min_lb55_vol10_bull", "timeframe": "15min", "lookback": 55, "volume_min": 1.0, "gate": "bull", "hold_max_bars": 20},
    {"name": "breakout_15min_lb20_vol10_bull", "timeframe": "15min", "lookback": 20, "volume_min": 1.0, "gate": "bull", "hold_max_bars": 20},
    {"name": "breakout_15min_lb20_vol12_bull", "timeframe": "15min", "lookback": 20, "volume_min": 1.2, "gate": "bull", "hold_max_bars": 20},
    {"name": "breakout_15min_lb55_vol12_bull", "timeframe": "15min", "lookback": 55, "volume_min": 1.2, "gate": "bull", "hold_max_bars": 20},
]
...
def main() -> None:
    ...
    event_cache = {
        candidate["name"]: {symbol: scan_events(fifteen[symbol], candidate) for symbol in symbols}
    ...
    for fold_name, dates in folds.items():
        ...
        for candidate in CANDIDATES:
            ...
            signals = 0
            for symbol in symbols:
                events = [event for event in event_cache[candidate["name"]][symbol] if dates[0] <= event["session_date"] <= dates[-1]]
                if candidate["gate"] == "bull":
                    events = [event for event in events if regimes.get(event["session_date"], {}).get("regime") == "bull"]
                signals += len(events)
            ...
            row = metric_row(aggregate_curves(curves), trades, candidate["name"], "15min", fold_name, signals)
            row["delta_return_pp"] = row["return_pct"] - base["return_pct"]
            row["delta_drawdown_pp"] = row["max_drawdown_pct"] - base["max_drawdown_pct"]
            rows.append(row)
    output = OUT_DIR / "breakout_20_55_walkforward_2026-08-19.csv"
    pd.DataFrame(rows).to_csv(output, index=False)
    with open(OUT_DIR / "breakout_20_55_walkforward_manifest_2026-08-19.json", "w", encoding="utf-8") as handle:
        json.dump({"folds": folds, "candidates": CANDIDATES, "symbols": symbols, "cost_bps_per_side": 5.0, "output": str(output)}, handle, indent=2)
