"""Backtest objective chart patterns over DayBreakout and standalone long signals."""
...
from strategies.chart_patterns import detect_all
...
ENTRY_START = "10:00"
ENTRY_END = "15:30"
VARIANTS = ["baseline", "double_bottom_filter", "triangle_filter", "flag_filter",
            "inverse_hs_filter", "pattern_confluence_filter", "bear_pattern_block",
            "double_bottom_standalone", "triangle_standalone", "flag_standalone",
            "inverse_hs_standalone"]
...
def _load(symbol: str) -> pd.DataFrame:
    ...
    signals = detect_all(frame)
    for column in signals:
        frame[f"pattern_{column}"] = signals[column]
...
def _pattern_allowed(row: pd.Series, variant: str, breakout: bool) -> bool:
    long_signal = {
        "double_bottom_filter": bool(row["pattern_double_bottom"]),
        "triangle_filter": row["pattern_triangle"] == 1,
        "flag_filter": row["pattern_flag"] == 1,
        "inverse_hs_filter": row["pattern_inverse_head_shoulders"] == 1,
        "pattern_confluence_filter": bool(row["pattern_double_bottom"])
        or row["pattern_triangle"] == 1
        or row["pattern_flag"] == 1
        or row["pattern_inverse_head_shoulders"] == 1,
        "bear_pattern_block": not (
            bool(row["pattern_double_top"])
            or row["pattern_triangle"] == -1
            or row["pattern_flag"] == -1
            or row["pattern_head_shoulders"] == -1),
    }
    standalone = variant.endswith("_standalone")
    if variant == "baseline":
        return breakout
    if standalone:
        mapping = {
            "double_bottom_standalone": "pattern_double_bottom",
            "triangle_standalone": "pattern_triangle",
            "flag_standalone": "pattern_flag",
            "inverse_hs_standalone": "pattern_inverse_head_shoulders",
        }
        signal = row[mapping[variant]]
        return signal == 1
    return breakout and long_signal[variant]


def _simulate(frame: pd.DataFrame, variant: str,
    ...
    for ts, row in data.iterrows():
        ...
        breakout = bool(pd.notna(row["donch_hi"]) and close > float(row["donch_hi"]))
        if shares == 0 and in_session and _pattern_allowed(row, variant, breakout):
...
def main() -> None:
    ...
    for window, (start, end) in _windows(last).items():
        for variant in VARIANTS:
            ...
            for symbol, frame in frames.items():
                trades, curve = _simulate(frame, variant, start, end)
    ...
    result.to_csv(OUT / "chart_pattern_backtests_2026-08-18_results.csv", index=False)
    manifest = {"symbols": SYMBOLS, "timeframe": "15min", "source": "Alpaca IEX",
                "capital": CAPITAL, "slippage_bps": SLIPPAGE_BPS, "variants": VARIANTS,
                "windows": list(_windows(last)), "confirmation": "closed pivot + ATR buffer",
                "coverage": "objective patterns; no visual/manual labels"}
    (OUT / "chart_pattern_backtests_2026-08-18_manifest.json").write_text(
