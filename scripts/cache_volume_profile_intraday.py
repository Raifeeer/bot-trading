"""Cache histórico intradía para la investigación de Volume Profile.

Lee las credenciales de Alpaca desde Secret Manager en memoria, descarga barras
15m por símbolo y guarda únicamente datos OHLCV normalizados. No coloca órdenes.
"""
...
OUT = Path("/home/ubuntu/backtests/volume_profile_history")
    ...
    for symbol in UNI_RETO:
        try:
            ...
            manifest.append({"symbol": symbol, "rows": len(df),
                             ...
                             "status": "ok"})
            print(f"{symbol}: {len(df)} barras 15m")
        except (RuntimeError, ValueError, OSError) as exc:  # proveedor externo
