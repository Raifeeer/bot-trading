"""Resumen reproducible de Williams %R versus RSI."""
...
def main() -> None:
    df = pd.read_csv(ROOT / "williams_r_backtests_2026-08-18_results.csv")
    metrics = ["return_pct", "drawdown_pct", "trades", "win_rate_pct", "profit_factor"]
    grouped = df.groupby(["window", "period", "variant"], as_index=False)[metrics].mean()
    bases = grouped[grouped["variant"] == "baseline"].set_index(["window", "period"])
    rows = []
    for _, row in grouped.iterrows():
        base = bases.loc[(row["window"], row["period"])]
    ...
    deltas.to_csv(ROOT / "williams_r_backtests_2026-08-18_deltas.csv", index=False)
    ...
    summary.to_csv(ROOT / "williams_r_backtests_2026-08-18_variant_summary.csv", index=False)
    robust = summary[(summary["windows_beating_return"] >= 8)
                     & (summary["windows_not_worse_drawdown"] >= 8)]
    robust.to_csv(ROOT / "williams_r_backtests_2026-08-18_robust_candidates.csv", index=False)
