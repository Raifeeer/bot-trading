"""Backtest VIX gates over DayBreakout using prior daily VIX observations."""
...
VIX_PATH = ROOT / "backtests/vix_history/vix.pkl"
OUT = ROOT / "backtests"
CAPITAL = 100_000.0
...
ENTRY_START = "10:00"
ENTRY_END = "15:30"
VARIANTS = ["baseline", "level_15", "level_20", "level_25", "level_30",
            "percentile_60", "percentile_70", "percentile_80",
            "shock_10", "shock_20", "shock_30", "zscore_2", "combined_25_shock"]
...
def _simulate(frame: pd.DataFrame, vix_features: pd.DataFrame, variant: str,
    ...
    prior_cache: dict[object, pd.Series | None] = {}
    for ts, row in data.iterrows():
        ...
        if (shares == 0 and in_session and pd.notna(row["donch_hi"])
            ...
            trade_date = ts.date()
            if trade_date not in prior_cache:
                prior_cache[trade_date] = prior_close_for_date(vix_features, trade_date)
            if not blocked_by_vix(prior_cache[trade_date], variant):
...
def main() -> None:
    ...
    vix_raw = pd.read_pickle(VIX_PATH)["close"]
    vix_features = daily_features(vix_raw)
    ...
    for window, (start, end) in _windows(last).items():
        for variant in VARIANTS:
            ...
            for symbol, frame in frames.items():
                trades, curve = _simulate(frame, vix_features, variant, start, end)
    ...
    result.to_csv(OUT / "vix_filter_backtests_2026-08-18_results.csv", index=False)
    manifest = {"symbols": SYMBOLS, "timeframe": "15min", "source": "Alpaca IEX + FRED VIXCLS",
                "capital": CAPITAL, "slippage_bps": SLIPPAGE_BPS, "variants": VARIANTS,
                "windows": list(_windows(last)), "vix_alignment": "prior_exchange_date_close",
                "coverage": "VIX level/percentile/shock; no VIX3M term structure"}
    (OUT / "vix_filter_backtests_2026-08-18_manifest.json").write_text(
