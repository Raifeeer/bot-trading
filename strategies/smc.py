...
Temporalidades de Jean Vizón:
  ...
  3. M15      = sesgo inmediato (CHoCH confirma fin de continuacion / inicio de retroceso)
  4. M1       = temporizacion de la entrada

Reglas:
  - Una tendencia en HTF debe suceder primero en LTF (flujo fractal).
  - CHoCH: señal de cambio en la estructura interna; exige confluencia
    (sweep de liquidez + OB de S&D / FVG) y nunca opera sola.
  - Mapeo conservador: mechas para estructura, cuerpos para BOS.
  - Zona de demanda: vela base contraria al impulso alcista con FVG (displacement).
  - Zona de supply: vela base contraria al impulso bajista con FVG.
...
@dataclass
class Zone:
    """Zona de S&D (order block con displacement / FVG)."""
...
def fractal_swing_points(df: pd.DataFrame, order: int = 3,
    ...
    for i in range(order, len(df) - order):
        win = df.iloc[i - order:i + order + 1]
        mid = df.iloc[i]

        is_h = mid.High == win.High.max()
        is_l = mid.Low == win.Low.min()

        if is_h and not is_l:
            swings.append(Swing(price=float(mid.High), idx=i, is_high=True))
        elif is_l and not is_h:
            swings.append(Swing(price=float(mid.Low), idx=i, is_high=False))

    return swings


def detect_choch(df: pd.DataFrame, order: int = 2):
    """Change of Character (CHoCH): rotura del ultimo alto/bajo estructural."""
...
def detect_sw_zone(df: pd.DataFrame, order: int = 2):
    """Zonas de supply/demanda con validacion de FVG (Fair Value Gap / Displacement).
    Demanda: vela bajista + FVG alcista 2 velas despues.
    Supply: vela alcista + FVG bajista 2 velas despues.
    """
    df = _norm(df)
    zones = []
    if len(df) < 4:
    ...
    for i in range(1, len(df) - 2):
        # Demanda: Vela bajista previa a impulso con FVG alcista
        if o[i] > c[i] and l[i + 2] > h[i]:
            zones.append(Zone("demand", max(o[i], c[i]), min(o[i], l[i]), i))
        # Supply: Vela alcista previa a colapso con FVG bajista
        elif c[i] > o[i] and h[i + 2] < l[i]:
            zones.append(Zone("supply", max(o[i], h[i]), min(o[i], c[i]), i))
...
class SMCStrategy:
    ...
    def trend_htf(self, df: pd.DataFrame) -> str:
        """Tendencia HTF: HH/HL (bull), LH/LL (bear) con fallback EMA 9/21 para series cortas."""
    ...
    def analyze(self, dfs: dict) -> dict:
        """Cascada MTF: 1d+4h -> niveles/tendencia; M15 -> sesgo (CHoCH); M1 -> timing."""
        result = {
        ...
        df_d, df_4h = (_norm(d) if d is not None else None for d in (df_d, df_4h))
        ...
        # 2) Sesgo inmediato: CHoCH en M15
            ...
            for z in zones:
                if z.bottom <= px <= z.top:
                    if (z.kind == "demand" and result["htf_trend"] == "bull"
                        ...
                        result["confluence"] += 0.15
                    result["note"] += f"precio en zona {z.kind} con FVG; "
