...
El ledger dedicado ya se consulta durante `load_state()`/BOOT y las órdenes abiertas se vuelven a consultar a Alpaca; `cbdc186` añade fusión segura con snapshots legados, halt cuando falla la lectura dedicada y CAS con `Client.write_option(last_update_time=...)` cuando existe timestamp. Las pruebas locales cubren doble claim, reinicio, fills/estados ambiguos, fallo al completar, snapshot legado, lectura dedicada fallida y precondición CAS. La producción confirmó la colección sin intents activos. La canary autorizada quedó encapsulada en `scripts/run_polaris_canary_once.py` y programada como ejecución única para `2026-08-25T13:30:05Z` (`America/Santo_Domingo`): una sola compra límite de `F260828C00015000`, 1 contrato, máximo `$0.02`; si llena, una sola venta límite al bid, y cualquier ambigüedad queda `needs_review` sin reintento. El runner verifica antes de enviar que `polaris-bot-cbdc186` conserve el 100% del tráfico. Aún no se ha probado una salida real con una posición PAPER, una caída de Firestore durante una pata real ni dos instancias simultáneas. El bot permanece clasificado como `CONTAINED_POST_LIQUIDATION_PENDING_REACTIVATION`/vigilancia segura y las entradas generales siguen bloqueadas. El rollback inmediato ante traceback, orden inesperada o divergencia es `polaris-bot-secretmigrate3` o, si fuera necesario, `polaris-bot-guarddddcda7` al 100%.

Pendientes posteriores: esperar y verificar el resultado de la canary programada; rotar criptográficamente `TELEGRAM_BOT_TOKEN` mediante un token nuevo de BotFather; probar caída de Firestore durante una pata real y concurrencia; y mantener todas las estrategias nuevas en `RESEARCH_ONLY`/shadow hasta evidencia reproducible. El runner one-shot y sus tests offline quedaron versionados hasta `6825f6c`; el procedimiento post-canary está en `docs/canary_post_execution_checklist.md`. Telegram ya muestra `new_entries_halted`, `broker_reconciliation_halt`, órdenes abiertas, intents y patas no gestionables. La estabilidad operativa no implica rentabilidad ni garantiza recuperar el capital perdido.
