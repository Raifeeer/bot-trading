...
El hotfix inicial `07517fb` fue extendido por `c9cbc67` y `8c5f1dc`. Está desplegada la revisión `polaris-bot-dedicated8c5f1dc`, con 100% del tráfico y digest `sha256:3ac6f75cb06b921e34620e5377621c160117cdfa3f72fd9106cbb22e5b7c73fe`. El código fuerza `risk.halt_new_entries=true`, restaura intents activos desde `polaris_exit_ledger`, consulta órdenes abiertas frescas del broker y bloquea entradas ante divergencias. La primera pata exige `create()` idempotente; cada pata actualiza el documento por ID y el cierre solo se marca `completed` después de confirmar broker y Firestore. Defined-risk multi-leg no se ejecuta: el executor sigue sin atomicidad real.

Tras el nuevo deploy se verificaron BOOT y tres ciclos completos. El snapshot real `polaris/2026-08-24` quedó actualizado a `2026-08-24T19:44:28.810535+00:00` y confirmó `trading_mode=PAPER`, equity `$96,915.63`, posiciones `0`, órdenes abiertas `0`, `exit_intents=0`, `new_entries_halted=true`, `broker_reconciliation_halt=false`, `approved=0`, `orders=0`, `unmanaged_broker_legs=0` y `unmanaged_state_positions=0`. La lectura REST de `polaris_exit_ledger` respondió correctamente con cero documentos activos. Los logs no mostraron tracebacks, excepciones críticas ni órdenes enviadas; solo warnings de datos SIP no permitidos con fallback real a yfinance.

### Bloqueo antes de reanudar

El ledger dedicado ya se consulta durante `load_state()`/BOOT y las órdenes abiertas se vuelven a consultar a Alpaca; las pruebas locales cubren doble claim, reinicio, fills/estados ambiguos y fallo al completar. La producción confirmó que la colección responde sin intents activos. Aún no se ha probado una salida real con una posición PAPER, una caída de Firestore durante una pata real ni dos instancias simultáneas. El bot permanece clasificado como `CONTAINED_POST_LIQUIDATION_PENDING_REACTIVATION`/vigilancia segura sin posiciones. No crear posiciones para probarlo, no reactivar entradas ni habilitar gestión automática de spreads hasta una canary PAPER deliberada, revisión humana y autorización nueva. El rollback inmediato ante traceback, orden inesperada o divergencia es `polaris-bot-guarddddcda7` al 100%.

Pendientes posteriores: migrar y rotar `TELEGRAM_BOT_TOKEN` desde el valor literal del spec a Secret Manager; ejecutar una canary PAPER deliberada con tamaño mínimo y autorización explícita; probar caída de Firestore durante una pata real y concurrencia; y mantener todas las estrategias nuevas en `RESEARCH_ONLY`/shadow hasta evidencia reproducible. Telegram ya muestra `new_entries_halted`, `broker_reconciliation_halt`, órdenes abiertas, intents y patas no gestionables. La estabilidad operativa no implica rentabilidad ni garantiza recuperar el capital perdido.
