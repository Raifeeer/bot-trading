...
El hotfix inicial `07517fb` fue extendido por `c9cbc67` y está desplegado en la revisión `polaris-bot-ledgerc9cbc67`, con 100% del tráfico y digest `sha256:091deb6747ea57658cb2f9235acaa398907caa1d3d70e669568fd2254ef96638`. El código fuerza `risk.halt_new_entries=true`, rehidrata `exit_intents`/`exit_history` desde Firestore en BOOT, consulta órdenes abiertas frescas del broker y bloquea entradas ante divergencias. Las salidas usan estados por ID, confirmación de desaparición de patas y `needs_review`/`partial_submission` sin reintentos automáticos. Defined-risk multi-leg no se ejecuta: el executor sigue sin atomicidad real.

Tras el nuevo deploy se verificó BOOT con `exit ledger restaurado desde Firestore día=2026-08-24 intents=0` y tres ciclos completos. El snapshot real `polaris/2026-08-24` quedó actualizado a `2026-08-24T19:18:02.770330+00:00` y confirmó `trading_mode=PAPER`, equity `$96,915.63`, posiciones `0`, órdenes abiertas `0`, `exit_intents=0`, `new_entries_halted=true`, `broker_reconciliation_halt=false`, `approved=0`, `orders=0`, `unmanaged_broker_legs=0` y `unmanaged_state_positions=0`. Los logs de la revisión no mostraron tracebacks, excepciones, críticos ni órdenes enviadas. Solo persistieron warnings de datos SIP no permitidos con fallback real a yfinance.

### Bloqueo antes de reanudar

El ledger ya se restaura desde Firestore durante `load_state()`/BOOT y las órdenes abiertas se consultan de nuevo a Alpaca; la prueba local determinista de reinicio pasó y la revisión real confirmó el log de restauración. Aun así, el ledger se guarda dentro de un snapshot con captura de excepciones, no en una colección transaccional dedicada, y faltan pruebas de escritura fallida y concurrencia. El bot permanece clasificado como `CONTAINED_POST_LIQUIDATION_PENDING_REACTIVATION`/vigilancia segura sin posiciones. No crear posiciones para probarlo, no reactivar entradas ni habilitar gestión automática de spreads hasta completar esas pruebas y una revisión humana. El rollback inmediato ante traceback, orden inesperada o divergencia es `polaris-bot-guarddddcda7` al 100%.

Pendientes posteriores: migrar y rotar `TELEGRAM_BOT_TOKEN` desde el valor literal del spec a Secret Manager; añadir una colección transaccional/dedicada para el ledger y pruebas de escritura fallida/concurrencia; y mantener todas las estrategias nuevas en `RESEARCH_ONLY`/shadow hasta evidencia reproducible. Telegram ya muestra `new_entries_halted`, `broker_reconciliation_halt`, órdenes abiertas, intents y patas no gestionables. La estabilidad operativa no implica rentabilidad ni garantiza recuperar el capital perdido.
