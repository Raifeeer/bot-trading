...
El hotfix inicial `07517fb` fue extendido por `c9cbc67`, `8c5f1dc`, `cbdc186` y `669a164`. Está desplegada la revisión `polaris-bot-669a164s2`, con 100% del tráfico y digest `sha256:9c698deec55707b15b709ddcea69a89f5e782ee758aba7cbd13702dd704c0256`; rollback inmediato `polaris-bot-cbdc186`. La configuración efectiva conserva `PAPER`, `minScale=1`, `maxScale=1` y CPU always-on. El código fuerza `risk.halt_new_entries=true`, restaura intents activos fusionando colección dedicada y snapshot legado, consulta órdenes abiertas frescas del broker y bloquea entradas ante divergencias o fallos de lectura. La primera pata exige `create()` idempotente; cada pata actualiza el documento por ID con precondición CAS cuando el SDK expone `update_time`; el cierre solo se marca `completed` después de confirmar broker y Firestore. Defined-risk multi-leg no se ejecuta: el executor sigue sin atomicidad real. Los helpers heredados `execute_trade.py` y `monitor_spread.py` están deshabilitados; calibradores y cotizadores ya no contienen credenciales literales.

Tras cbdc186 se verificaron BOOT y tres ciclos completos. El snapshot real `polaris/2026-08-24` quedó actualizado a `2026-08-24T21:39:24.725107+00:00` y confirmó `trading_mode=PAPER`, equity `$96,915.63`, posiciones `0`, órdenes abiertas `0`, `exit_intents=0`, `new_entries_halted=true`, `broker_reconciliation_halt=false`, `approved=0`, `orders=0`, `unmanaged_broker_legs=0` y `unmanaged_state_positions=0`. La configuración efectiva enlaza `APCA_API_KEY_ID`, `APCA_API_SECRET_KEY`, `DEEPSEEK_API_KEY` y `TELEGRAM_BOT_TOKEN` a Secret Manager; el token Telegram se migró sin cambiar su valor operativo. Los logs de cbdc186 registraron dos ticks completos y dos escrituras Firestore, sin tracebacks, excepciones, críticos, errores de permisos ni envíos de órdenes. Tras activar 669a164s2, se verificaron tres ciclos adicionales, Firestore coherente y ningún nuevo 409 de Telegram después de la transición inicial. La colección `polaris_exit_ledger` no tenía intents activos.

### Bloqueo antes de reanudar

El ledger dedicado ya se consulta durante `load_state()`/BOOT y las órdenes abiertas se vuelven a consultar a Alpaca; `cbdc186` añade fusión segura con snapshots legados, halt cuando falla la lectura dedicada y CAS con `Client.write_option(last_update_time=...)` cuando existe timestamp. `669a164` bloquea además si la lectura de posiciones de Alpaca produce timeout o error, evitando tratar una cuenta no confirmada como vacía. Las pruebas locales cubren doble claim, reinicio, fills/estados ambiguos, fallo al completar, snapshot legado, lectura dedicada fallida y precondición CAS. La producción confirmó la colección sin intents activos. La canary autorizada se intentó el `2026-08-25` durante mercado abierto con una sola compra límite de `F260828C00015000`, 1 contrato, máximo `$0.02`. El ask real fue `$0.04`; el runner abortó antes de enviar, sin IDs de órdenes, posiciones ni órdenes abiertas. Firestore quedó en `aborted_entry_quote_above_limit`, la programación one-shot fue desactivada y el bot permanece clasificado como `CONTAINED_POST_LIQUIDATION_PENDING_REACTIVATION`/vigilancia segura. El runner verifica antes de enviar que `polaris-bot-cbdc186` conserve el 100% del tráfico. Aún no se ha probado una salida real con una posición PAPER, una caída de Firestore durante una pata real ni dos instancias simultáneas. Las entradas generales siguen bloqueadas. El rollback inmediato ante traceback, orden inesperada o divergencia es `polaris-bot-secretmigrate3` o, si fuera necesario, `polaris-bot-guarddddcda7` al 100%.

Pendientes posteriores: obtener autorización explícita para una nueva canary solo si se acepta un precio actualizado; rotar criptográficamente `TELEGRAM_BOT_TOKEN` mediante un token nuevo de BotFather; probar caída de Firestore durante una pata real y concurrencia; publicar en GitHub los commits locales hasta `669a164` y este handoff cuando se restaure la autenticación; y mantener todas las estrategias nuevas en `RESEARCH_ONLY`/shadow hasta evidencia reproducible. El runner one-shot y sus tests offline quedaron validados con 206 tests; el procedimiento post-canary está en `docs/canary_post_execution_checklist.md`. Telegram ya muestra `new_entries_halted`, `broker_reconciliation_halt`, órdenes abiertas, intents y patas no gestionables. La estabilidad operativa no implica rentabilidad ni garantiza recuperar el capital perdido.


## Auditoría de autonomía y readiness — 25 de agosto de 2026

La revisión activa pasó a `polaris-bot-669a164s2`, imagen `sha256:9c698deec55707b15b709ddcea69a89f5e782ee758aba7cbd13702dd704c0256`, con configuración PAPER, `minScale=1`, CPU always-on y las cuatro variables sensibles desde Secret Manager. El cambio `669a164` hace fail-closed la reconciliación cuando Alpaca no responde o devuelve error al leer posiciones. La producción confirmó arranque, restauración del ledger, tres ciclos, cero posiciones, cero órdenes y `new_entries_halted=true`; el único 409 de Telegram quedó limitado a la transición inicial.

La canary del 25-ago abortó porque el ask de `F260828C00015000` fue `$0.04` frente al límite `$0.02`, sin crear una orden. La programación quedó pausada. El runner y sus tests cubren mercado cerrado, cuenta no vacía, revisión no contenida, claim previo, conflicto CAS, fallo de Firestore, límites de cotización y ausencia de reintentos. La suite global alcanza 208 tests tras el guard de reconciliación.
...
## 25. Piloto OOS Databento y estado actual — 25 de agosto de 2026

Se investigaron Databento OPRA, algoseek Options, Cboe DataShop y ThetaData. La recomendación preliminar es empezar con una muestra limitada de Databento OPRA por su API/bulk, last sale, exchange BBO, National BBO, instrument definitions y precio histórico publicado desde $0.04/GB; algoseek queda como alternativa de mayor fidelidad y mayor coste. Cboe DataShop es adecuado para intervalos de 1 minuto; ThetaData requiere confirmar precio, licencia y continuidad. La evidencia y referencias están en `docs/options_oos_provider_comparison_2026-08-25.md`.

El adaptador opcional `research/options_oos_databento.py`, el piloto `scripts/run_databento_oos_pilot.py` y `tests/test_options_oos_databento.py` quedaron añadidos. El piloto exige simultáneamente `--execute` y `DATABENTO_API_KEY`; sin ambos devuelve `blocked_no_network` y no realiza consultas. Normaliza `cbbo-1m/cbbo-1s` y definiciones de instrumentos, comprueba bid/ask positivos y no cruzados, duplicados, timestamps, tamaños opcionales y definiciones point-in-time, y escribe un manifest con hashes. No importa el bot, no cambia `config/config.yaml` y no puede enviar órdenes.
...
## 15. Preflight persistente Cloud Run validado — 27 de agosto de 2026 UTC

Esta sección supersede los estados de canary anteriores cuando exista contradicción. La tarea Manus `SX5nHwM1VFC9LKEtbruh1Q` sigue pausada: falló por aislamiento de workspace y no debe reactivarse ni duplicarse. La ruta operativa corregida es un Cloud Run Job separado, no una tarea Manus con `runAsNewTask=true`.

El runner `scripts/run_polaris_canary_auto_once.py` se publicó en `18e99cd`. La imagen de canary está fijada en `us-central1-docker.pkg.dev/gen-lang-client-0746441136/polaris-images/polaris-canary:18e99cd@sha256:035cb0304ced6a293f924d3893157e4dfeefe60c43e1015e49d0f9047012c3d4`. El job `polaris-canary-preflight` usa una task, `max-retries=0`, timeout de 10 minutos, la Compute default service account, secretos Alpaca por Secret Manager y las variables `GOOGLE_CLOUD_PROJECT`, `CLOUD_RUN_REGION`, `POLARIS_SERVICE`, `POLARIS_EXPECTED_REVISION`, `CANARY_RUN_ROOT`, `CANARY_PREFLIGHT_ONLY=1` y un `CANARY_RUN_ID` único.

La ejecución `polaris-canary-preflight-s9ftb` terminó `Completed=True`. El documento `polaris_canary_runs/canary-preflight-cloudjob-2026-08-27-v3` quedó `status=preflight_only`, `mode=PAPER`, `orders_allowed=false` y `production_config_changed=false`. La ejecución verificó Cloud Run `polaris-bot-telegramrotate2` al 100%, la cuenta Alpaca `ACTIVE`, equity/cash `$96,914.08`, mercado abierto, Firestore y una vertical válida de calls sobre F con débito `$0.01` y riesgo máximo de prima `$1`. Después de la prueba: 0 posiciones, 0 órdenes abiertas y 0 client IDs `polaris-auto-*`. Esto demuestra acceso persistente a secretos, ADC, Cloud Run, Logging, Firestore, Alpaca y quotes sin enviar órdenes.

La compuerta de salud no debe volver a bloquear por el warning conocido de suscripción SIP si el fallback real a yfinance está funcionando y hay telemetría fresca. Ahora contabiliza esos eventos como `handled_feed_fallback_count`; exige `Tick OK` y escritura Firestore recientes, y sigue abortando ante `Traceback`, `CRITICAL`, errores explícitos o `409 Conflict`. El preflight observó 9 ticks, 9 escrituras Firestore y 65 fallbacks manejados. La suite completa tras este cambio quedó en `238 passed, 1 skipped, 2 xfailed`; Ruff, compilación y `git diff --check` limpios.

### Procedimiento para una canary operativa posterior

No ejecutar todavía la canary real por este resultado. Antes de hacerlo, verificar de nuevo la cuenta PAPER, posiciones/órdenes, tráfico de `polaris-bot-telegramrotate2`, ventana de mercado y logs frescos. Actualizar **solo** el job `polaris-canary-paper` a una imagen inmutable que incluya la corrección, preservar service account, secretos, una task, cero reintentos y timeout, retirar `CANARY_PREFLIGHT_ONLY` únicamente con autorización vigente y usar un `CANARY_RUN_ID` nuevo. La orden permitida sigue siendo exactamente una vertical MLeg de calls de 2 patas, qty 1, débito máximo `$0.20` por acción, pérdida de prima máxima `$20` más comisiones, `DAY`, 120 s para entrada y salida, sin patas individuales ni reintentos ambiguos, cierre automático si llena y `needs_review` en cualquier parcial, error, ambigüedad o divergencia. Después hay que comprobar el documento Firestore, los dos posibles client IDs, fills, posición plana y cero órdenes abiertas.

El servicio productivo sigue con `risk.halt_new_entries=true`; no cambiar `config/config.yaml`, no activar REAL, no ampliar capital ni promover capas shadow. OOS continúa `RESEARCH_ONLY`. El token Telegram de la rotación v2 está comprometido por exposición en la conversación: pedir otro token a BotFather y rotarlo antes de considerar la canary operativa; nunca imprimir ni documentar ningún valor del token.


### Job PAPER armado, todavía bloqueado — 27 de agosto de 2026 UTC

`polaris-canary-paper` quedó actualizado a la imagen inmutable `us-central1-docker.pkg.dev/gen-lang-client-0746441136/polaris-images/polaris-canary:18e99cd@sha256:035cb0304ced6a293f924d3893157e4dfeefe60c43e1015e49d0f9047012c3d4`. Conserva una task, `maxRetries=0`, timeout 600 s, comando `python3 scripts/run_polaris_canary_auto_once.py`, Compute default service account y referencias de Secret Manager para Alpaca. Tiene `CANARY_PREFLIGHT_ONLY=1` y el ID `canary-paper-armed-preflight-2026-08-27`; no se ha ejecutado.

No retirar el modo preflight ni invocar este job hasta recibir confirmación explícita vigente de la canary operativa. La confirmación debe repetir: Alpaca PAPER, una sola vertical MLeg de calls de 2 patas, qty 1, débito máximo `$0.20` por acción, pérdida máxima de prima `$20` más comisiones, `DAY`, 120 s para entrada/salida, sin fallback de patas ni reintentos ambiguos y cierre automático si llena. Para la ejecución real se debe usar un `CANARY_RUN_ID` nuevo. Mantener pausada la tarea Manus anterior y conservar `risk.halt_new_entries=true` en el servicio productivo.


## 16. Resultado de la canary PAPER única — 27 de agosto de 2026 UTC

La canary operativa confirmada se ejecutó una sola vez mediante `polaris-canary-paper`, ejecución `polaris-canary-paper-g2v89`. El primer intento posterior a la confirmación abortó antes de enviar por un defecto de empaquetado (`ModuleNotFoundError: No module named 'execution'`); se corrigió el `sys.path` del runner, se pasó la suite completa y se usó un `CANARY_RUN_ID` nuevo. No hubo orden ni posición del primer intento.

La ejecución efectiva terminó correctamente y el runner quedó en `entry_not_filled`. Seleccionó una vertical call MLeg sobre F: compra `F260925C00017000`, venta `F260925C00017500`, qty 1, débito `$0.01` por acción, riesgo máximo de prima `$1`, `DAY`. Envió exactamente una orden padre MLeg `55342ded-a492-4023-b367-24cf01d88137` con client ID `polaris-auto-20260827-43d962a96c37874ee42c4d36`; ambas patas eran 1:1. No hubo fill: la orden padre y las dos patas terminaron `canceled`, `filled_qty=0`, y no se envió una orden de salida.

La comprobación posterior encontró cuenta PAPER `ACTIVE`, equity/cash `$96,914.08`, cero posiciones, cero órdenes abiertas y exactamente una orden histórica `polaris-auto-*`, correspondiente a esta canary cancelada. No hubo exposición ni impacto de P&L. El documento `polaris_canary_runs/canary-paper-live-2026-08-27-1505Z` conserva la selección y la auditoría; `orders_allowed` fue corregido a `false` mediante CAS. El job se devolvió a `CANARY_PREFLIGHT_ONLY=1`, con ID `canary-paper-locked-after-run-2026-08-27`, una task, cero reintentos, timeout 600 s y la imagen final `polaris-canary:aef7bbd@sha256:f6b4264810db0a4dbed275f9d0c69ecf422b8239d078aa19211c157d8b70e688`.

La suite completa quedó en `239 passed, 1 skipped, 2 xfailed`, con Ruff, compilación y `git diff --check` limpios. El resultado valida la ruta básica MLeg PAPER y la cancelación sin fill, pero no demuestra rentabilidad ni autoriza otra canary, más capital, promoción de estrategias shadow, cambios de riesgo o REAL. El servicio productivo conserva 100% del tráfico en `polaris-bot-telegramrotate2` y `risk.halt_new_entries=true`. El token Telegram expuesto en la conversación sigue requiriendo rotación adicional vía BotFather.


## 17. Activación PAPER gradual — 27 de agosto de 2026 UTC

Con confirmación explícita del usuario, Polaris quedó habilitado para entradas nuevas únicamente en PAPER. La política vigente es: una posición abierta máxima, pérdida diaria absoluta máxima de `$20`, `universo.options_reto.max_premium_net=12.0`, dirección bull, sin REAL, sin capas shadow/promoted con autoridad de órdenes y sin cambiar los guardarraíles de floor, RiskManager, circuit breakers, earnings, validación de quotes, ledger o idempotencia. El commit de configuración es `f409468` (`halt_new_entries=false`, `max_open_positions=1`, `max_daily_loss_usd=20.0`).

La revisión activa es `polaris-bot-00120-lxz`, con 100% del tráfico, imagen inmutable `polaris-bot:f409468@sha256:4f97db2f4e728095629bff136c3cac354653e9c44c1a9f5ac886c19872f76e6d`, PAPER, concurrencia 1, una instancia, CPU always-on y polling de 60 s. En tres ciclos verificados entre 15:41:48 y 15:43:53 UTC se observó `new_entries_halted=false`, `broker_reconciliation_halt=false`, cero órdenes broker abiertas, cero patas no gestionadas, cero posiciones no gestionadas, 3 escrituras Firestore, 3 `Tick OK` y ningún Traceback/CRITICAL en la revisión nueva.

No se enviaron órdenes porque los tres ciclos tuvieron `tradable=0`, `approved=0` y `orders=0`. El bot está habilitado para entrar cuando exista una señal válida; no se debe fabricar una operación para provocar actividad. Persisten warnings conocidos de SIP sin entitlement y símbolo `^VIX` inválido, sin evidencia en esta ventana de fallo crítico de ejecución. El job `polaris-canary-paper` sigue bloqueado en `CANARY_PREFLIGHT_ONLY=1`. El token Telegram v2 sigue comprometido y requiere rotación adicional vía BotFather.


## 18. Override bajista experimental PAPER — 27 de agosto de 2026 UTC

El motor bajista de put spreads definidos quedó habilitado en PAPER mediante el commit `565bc70`. El override autorizado cambia `options_bear.enabled=true`, elimina solo el mínimo histórico de prima (`min_premium_net=0.0`) y aplica un máximo duro de `max_premium_net=20.0`; el límite propio del motor es una posición y el límite global también es una posición. Se conservan DTE 14–35, put vertical MLeg 1:1, TP 1.5x, SL 0.50x, RiskManager, floor, circuit breakers, filtro de earnings, validación de bid/ask, ledger y no reintentos ambiguos. No se habilitan puts desnudos, cortos de acciones, REAL ni shadow como autoridad.

La imagen `polaris-bot:565bc70@sha256:093e13ecec0e784d8e6add77bdb9f9532c1fc7402c08bd5e5b5b217face0d637` está desplegada al 100% como revisión `polaris-bot-00121-7x4`, con rollback disponible a la revisión anterior. La observación inicial registró `new_entries_halted=false`, `broker_reconciliation_halt=false`, 2 `Tick OK`, 2 escrituras Firestore, cero Tracebacks/CRITICAL/409, cero órdenes y cero posiciones. `bear_candidates=0` en la ventana: no apareció una señal bajista CHoCH elegible y no se fabricó actividad.

La cuenta PAPER permaneció `ACTIVE`, equity/cash `$96,914.08`, 0 posiciones y 0 órdenes abiertas. El override debe considerarse experimental y no validado para rentabilidad: el backtest previo favorecía una prima mínima de `$100` por el efecto de comisiones, por lo que una operación inferior puede tener expectativa económica desfavorable aunque respete el riesgo máximo. Si aparecen datos críticos, divergencia broker/Firestore, fill parcial, estado desconocido o incumplimiento del tope, detener entradas y hacer rollback.


## 19. Tope bajista PAPER aumentado a $50 — 27 de agosto de 2026 UTC

El usuario confirmó elevar a `$50` la prima máxima del motor bajista experimental. El commit `89f18bd` cambia solo `options_bear.max_premium_net` a `50.0`; se conservan el mínimo experimental `0.0`, una posición total máxima, `risk.max_daily_loss_usd=20.0`, `new_entries_halted=false`, put vertical MLeg 1:1, TP/SL 1.5x/0.50x, DTE 14–35, reconciliación, ledger e idempotencia. Una operación individual puede perder hasta `$50` más comisiones; el límite diario de `$20` bloquea entradas posteriores, pero no limita retrospectivamente una pérdida individual ya realizada.
...
## Actualización 27-ago-2026 — motores adicionales PAPER

El commit `8052bde` conecta tres motores adicionales a la ruta MLeg PAPER: `regime_aware` (fallback bull basado en régimen y RSI), `regime_hold_cash + put_choch` (put spread definido solo en régimen bear con CHoCH) y `relative_strength` (motor independiente: call spread para líderes bull, put spread para laggards bear). Todos usan la misma validación de quotes, `RiskManager`, claim CAS del entry ledger, idempotencia, reconciliación y fail-closed. La prioridad evita simultaneidad y deja una única posición MLeg total.

La política efectiva permanece PAPER-only: máximo `$50` para puts bear, `$12` para calls bull, pérdida diaria absoluta `$20`, una posición, TP/SL por prima, timeout/cancelación, floor y circuit breakers. No se habilitan acciones short, opciones desnudas, REAL ni capas shadow sin promoción específica. `relative_strength` no opera en régimen `cash` y no fabrica una señal si faltan datos o la dirección global no coincide.

La imagen `polaris-bot:8052bde@sha256:c94cda973d5b783e8718b3869b2163913e443a80ef3e572a48bb28871fbfb8ac` quedó al 100% en `polaris-bot-00123-8fv`. La primera observación registró arranque correcto, `ACTIVE` PAPER, equity/cash `$96,914.08`, régimen `cash` (bull 3/8, bear 0/8), `bear_candidates=0`, `regime_aware_candidates=0`, `relative_strength_candidates=0`, sin órdenes ni posiciones. En régimen lateral no debe operar ningún motor.

Las pruebas quedaron en `244 passed, 1 skipped, 2 xfailed`; la campaña de backtest permanece `RESEARCH_ONLY`. No usar los retornos históricos como garantía de rentabilidad. Para una señal bajista futura, verificar especialmente quotes OPRA, liquidez, prima total, fill del padre MLeg, ledger y stop/TP; cualquier respuesta ambigua mantiene el halt de reconciliación.
