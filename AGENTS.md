...
|---|---|
| Repositorio | `Raifeeer/bot-trading`, rama `main`; Cloud Run ejecuta `cbdc186`; herramientas de canary/seguridad versionadas hasta `8f2f705`; handoff actualizado en `2a671f2` |
...
Rollback inmediato si aparece cualquier orden inesperada, traceback de ejecución, divergencia broker/estado o nuevo fill no autorizado: volver el 100% del tráfico a `polaris-bot-secretmigrate3` o, si fuera necesario, a `polaris-bot-guarddddcda7`, sin crear posiciones para probar. La migración del token a Secret Manager está completada; su rotación criptográfica requiere un token nuevo de BotFather. La canary fue autorizada para el contrato y límite indicados, pero aún no ha ejecutado la orden: la programación one-shot está activa y expira `2026-08-25T14:00:00Z`. El runner exige además que `polaris-bot-cbdc186` conserve el 100% del tráfico antes de enviar. Los comandos Telegram ya muestran la contención publicada.
