...
|---|---|
| Repositorio | `Raifeeer/bot-trading`, rama `main`; hotfix de código `07517fb`; documentar y verificar el commit de handoff posterior |
| Cloud Run | Servicio `polaris-bot`, us-central1, proyecto `gen-lang-client-0746441136`; revisión `polaris-bot-idempotent07517` con 100% del tráfico, digest `sha256:898761ed424e26d2cb504a8c9177ea75238325f4326ba220fbdc5ba6eec95ead` |
...
| Seguridad | Wrappers shadow fuerzan `mode=shadow`, `influence_entries=false` y `orders_allowed=false`; RiskManager, floor, circuit breakers, límites de posiciones y cotizaciones son autoridad final. `07517fb` añade reconciliación recurrente y bloqueo fail-closed por órdenes abiertas/fallo de estado |
| Ledger de salidas | `c9cbc67` rehidrata `exit_intents`/`exit_history` desde Firestore en BOOT, consulta órdenes abiertas frescas al broker y mantiene fail-closed; aún no hay colección transaccional dedicada, por lo que no reactivar operativa |
...
El commit `07517fb` implementó el hotfix de vigilancia: `risk.halt_new_entries=true`, reconciliación en cada ciclo, consulta de órdenes abiertas de Alpaca antes de aprobar entradas, bloqueo ante fallo de reconciliación u órdenes abiertas, ledger `exit_intents`/`exit_history`, consulta de estados por ID, confirmación de desaparición de patas antes de retirar una posición, y estado `needs_review`/`partial_submission` sin reintento automático. También publica IDs y estados de órdenes, contadores de reconciliación y contención en Firestore/Telegram. Defined-risk multi-leg continúa fuera de ejecución porque el executor no tiene atomicidad real.

La imagen del hotfix inicial se construyó con digest `sha256:898761ed424e26d2cb504a8c9177ea75238325f4326ba220fbdc5ba6eec95ead`; la imagen vigente de `c9cbc67` usa digest `sha256:091deb6747ea57658cb2f9235acaa398907caa1d3d70e669568fd2254ef96638` y la revisión `polaris-bot-ledgerc9cbc67` recibe el 100% del tráfico. Tras el nuevo deploy se verificó BOOT con `exit ledger restaurado desde Firestore`, tres ciclos (`Tick OK`) y el snapshot `polaris/2026-08-24` con `updated_at=2026-08-24T19:18:02.770330+00:00`, modo `PAPER`, equity `$96,915.63`, posiciones `0`, `open_broker_orders=0`, `exit_intents=0`, `new_entries_halted=true`, `broker_reconciliation_halt=false`, `tick_diagnostics.approved=0`, `tick_diagnostics.orders=0`, `unmanaged_broker_legs=0` y `unmanaged_state_positions=0`. Los logs de la revisión no mostraron tracebacks, excepciones, críticos ni órdenes enviadas; los warnings SIP de Alpaca y fallback a yfinance son de datos, no de ejecución.

**Limitación crítica todavía abierta:** `c9cbc67` ya restaura `exit_intents`/`exit_history` desde el snapshot Firestore durante BOOT y consulta de nuevo las órdenes abiertas a Alpaca; la revisión activa confirmó el log `exit ledger restaurado desde Firestore`, tres ciclos limpios y cero intents. La escritura sigue siendo un snapshot con captura de excepciones, no una colección transaccional dedicada, y no existe todavía una prueba de reinicio contra una escritura fallida o dos instancias concurrentes. Por tanto, el sistema es seguro para vigilancia contenida sin posiciones, pero no se debe reactivar la operativa automática hasta completar esas pruebas y una revisión humana.

Rollback inmediato si aparece cualquier orden inesperada, traceback de ejecución, divergencia broker/estado o nuevo fill no autorizado: volver el 100% del tráfico a `polaris-bot-guarddddcda7`, sin crear posiciones para probar. Antes de cualquier reanudación se requieren una prueba de escritura fallida/reinicio, una revisión de concurrencia, pruebas de fill parcial/orden abierta y revisión humana con nueva autorización explícita. Pendientes secundarios: migrar y rotar `TELEGRAM_BOT_TOKEN` desde el valor literal del spec a Secret Manager; los comandos Telegram ya muestran la contención publicada.
