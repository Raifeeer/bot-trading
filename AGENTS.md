...
| Ledger de salidas | `07517fb` registra `exit_intents`/`exit_history`, estados por ID y fills parciales en el snapshot, pero todavía no restaura el ledger desde Firestore al reiniciar; mantener contención hasta completar persistencia durable |
...
La imagen se construyó correctamente con digest `sha256:898761ed424e26d2cb504a8c9177ea75238325f4326ba220fbdc5ba6eec95ead`. La revisión `polaris-bot-idempotent07517` recibe el 100% del tráfico en Cloud Run. Después del despliegue se observaron al menos seis ciclos; el último snapshot verificado en `polaris/2026-08-24` tenía `updated_at=2026-08-24T18:56:02.026266+00:00`, modo `PAPER`, equity `$96,915.63`, posiciones `0`, `open_broker_orders=0`, `exit_intents=0`, `new_entries_halted=true`, `broker_reconciliation_halt=false`, `tick_diagnostics.approved=0`, `tick_diagnostics.orders=0`, `unmanaged_broker_legs=0` y `unmanaged_state_positions=0`. Los logs mostraron `Tick OK`, escritura Firestore y cero tracebacks. Hubo un `Telegram poll ... HTTP 409` aislado y warnings esperables del plan de datos SIP de Alpaca con fallback a yfinance; ninguno produjo órdenes ni detuvo el tick.

**Limitación crítica todavía abierta:** el ledger se escribe dentro del snapshot Firestore, pero `load_state()` aún restaura `exit_intents`, `exit_history` y órdenes desde el filesystem efímero local; no los restaura desde Firestore al arrancar. Por tanto, el sistema está seguro para vigilancia contenida sin posiciones, pero no se debe afirmar idempotencia durable ni reactivar gestión automática de salidas/entradas hasta implementar la restauración persistente y sus pruebas de reinicio.

Rollback inmediato si aparece cualquier orden inesperada, traceback de ejecución, divergencia broker/estado o nuevo fill no autorizado: volver el 100% del tráfico a `polaris-bot-guarddddcda7`, sin crear posiciones para probar. Antes de cualquier reanudación se requieren persistencia durable del ledger, pruebas de reinicio/fill parcial/orden abierta y revisión humana con nueva autorización explícita. Pendientes secundarios: migrar y rotar `TELEGRAM_BOT_TOKEN` desde el valor literal del spec a Secret Manager y mejorar la visualización de contención en los comandos Telegram.
