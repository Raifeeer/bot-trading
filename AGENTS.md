...
El commit `07517fb` implementó el hotfix de vigilancia: `risk.halt_new_entries=true`, reconciliación en cada ciclo, consulta de órdenes abiertas de Alpaca antes de aprobar entradas, bloqueo ante fallo de reconciliación u órdenes abiertas, ledger local `exit_intents`/`exit_history`, consulta de estados por ID, confirmación de desaparición de patas antes de retirar una posición y estado `needs_review`/`partial_submission` sin reintento automático. `8c5f1dc` añade la colección dedicada `polaris_exit_ledger`, reclamación no sobrescribible y actualización versionada por pata. Defined-risk multi-leg continúa fuera de ejecución porque el executor no tiene atomicidad real.

La imagen del hotfix inicial se construyó con digest `sha256:898761ed424e26d2cb504a8c9177ea75238325f4326ba220fbdc5ba6eec95ead`; la imagen intermedia `c9cbc67` usó `sha256:091deb6747ea57658cb2f9235acaa398907caa1d3d70e669568fd2254ef96638`; la imagen vigente de `8c5f1dc` usa digest `sha256:3ac6f75cb06b921e34620e5377621c160117cdfa3f72fd9106cbb22e5b7c73fe` y la revisión `polaris-bot-dedicated8c5f1dc` recibe el 100% del tráfico. Tras el nuevo deploy se verificó BOOT, tres ciclos (`Tick OK`) y Firestore con `updated_at=2026-08-24T19:44:28.810535+00:00`, modo `PAPER`, equity `$96,915.63`, posiciones `0`, `open_broker_orders=0`, `exit_intents=0`, `new_entries_halted=true`, `broker_reconciliation_halt=false`, `tick_diagnostics.approved=0`, `tick_diagnostics.orders=0`, `unmanaged_broker_legs=0` y `unmanaged_state_positions=0`. La colección dedicada respondió correctamente con `0` documentos activos. No hubo tracebacks, excepciones críticas ni órdenes enviadas.

**Limitación crítica todavía abierta:** `8c5f1dc` ya usa la colección dedicada y las pruebas deterministas cubren doble claim, actualización versionada, reinicio, fills/estados ambiguos y fallo de completion. La revisión real confirmó que la colección responde y no contiene intents activos. Aún no se ha probado una salida real con una posición PAPER, ni una caída de Firestore durante una pata real, ni dos instancias Cloud Run simultáneas; por tanto, no se debe reactivar la operativa automática hasta completar esas pruebas y una revisión humana.

Rollback inmediato si aparece cualquier orden inesperada, traceback de ejecución, divergencia broker/estado o nuevo fill no autorizado: volver el 100% del tráfico a `polaris-bot-guarddddcda7`, sin crear posiciones para probar. Antes de cualquier reanudación se requiere una canary PAPER deliberada y explícitamente autorizada, con tamaño mínimo, además de revisión humana. Pendiente secundario: migrar y rotar `TELEGRAM_BOT_TOKEN` desde el valor literal del spec a Secret Manager; los comandos Telegram ya muestran la contención publicada.
