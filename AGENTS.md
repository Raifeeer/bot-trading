...
## 15. Estado vigente posterior al preflight Cloud Run — 27 de agosto de 2026 UTC

Esta entrada supersede los estados operativos anteriores de esta guía cuando exista contradicción. El repositorio `Raifeeer/bot-trading` está en `main`, commit publicado `18e99cd`, con árbol limpio al documentar este estado. El servicio productivo `polaris-bot` continúa en Alpaca PAPER, con 100% del tráfico en `polaris-bot-telegramrotate2`, imagen estable MLeg `sha256:5da658c626ed87c16d9418397beea36b85b49eb3c3fdc46022519b22624e71d1`, `risk.halt_new_entries=true`, sin cambios en `config/config.yaml`.

Se corrigió la arquitectura de ejecución de la canary. El runner `scripts/run_polaris_canary_auto_once.py` ya no depende de un workspace de la sandbox ni de `gcloud` dentro del contenedor: usa secretos inyectados o Secret Manager REST, ADC/OAuth para Firestore, Cloud Run y Cloud Logging, y un `CANARY_RUN_ID` único. La verificación de Cloud Run usa el endpoint regional v2 oficial y admite la forma v2 de tráfico/containers. El job separado `polaris-canary-preflight` está fijado a la imagen `us-central1-docker.pkg.dev/gen-lang-client-0746441136/polaris-images/polaris-canary:18e99cd@sha256:035cb0304ced6a293f924d3893157e4dfeefe60c43e1015e49d0f9047012c3d4`, una task, cero reintentos, timeout 10 minutos, service account Compute default, y `CANARY_PREFLIGHT_ONLY=1`.

La ejecución `polaris-canary-preflight-s9ftb` terminó correctamente. El documento `polaris_canary_runs/canary-preflight-cloudjob-2026-08-27-v3` quedó en estado `preflight_only`, modo `PAPER`, `orders_allowed=false` y `production_config_changed=false`. Verificó la revisión productiva esperada, cuenta Alpaca `ACTIVE`, equity/cash `$96,914.08`, mercado abierto, Firestore y selección de una vertical válida (`F260918C00165000`/`F260918C00180000`, débito `$0.01`, riesgo de prima `$1`); no ejecutó POST de órdenes por diseño. La comprobación posterior de Alpaca confirmó 0 posiciones, 0 órdenes abiertas y 0 IDs `polaris-auto-*`.

La compuerta de salud fue ajustada con evidencia. Los warnings conocidos `subscription does not permit querying recent SIP data` y el fallback a yfinance se registran como `handled_feed_fallback_count`, pero no bloquean por sí solos una canary de lifecycle cuando existen telemetría fresca. Siguen siendo críticos `Traceback`, `CRITICAL`, errores explícitos y conflictos 409; además, la compuerta exige al menos un `Tick OK` y una escritura Firestore reciente. El preflight v3 observó 9 ticks, 9 escrituras Firestore y 65 warnings de fallback manejado, sin marcadores críticos. La suite completa quedó en `238 passed, 1 skipped, 2 xfailed`, con Ruff, compilación y `git diff --check` limpios.
...
La canary autorizada se ejecutó una sola vez con el job aislado `polaris-canary-paper`, ejecución `polaris-canary-paper-g2v89`, sobre la imagen `polaris-canary:afdad6f@sha256:586ae6c3dd704e2f0e8b0a0300ee8b8d5f3508629974b2f30a94749e6916f622`. El primer intento con la misma autorización abortó antes del envío por un defecto de empaquetado (`ModuleNotFoundError: No module named 'execution'`); no creó orden ni posición. Se corrigió el import para contenedor, se validó la suite completa y se volvió a ejecutar una única vez con un ID distinto.

La ejecución efectiva terminó `Completed=True` con estado Firestore `entry_not_filled`. La estructura seleccionada fue una vertical call MLeg sobre F: compra de `F260925C00017000` y venta de `F260925C00017500`, qty 1, débito límite `$0.01` por acción y riesgo máximo de prima `$1`; cumplía ampliamente el límite autorizado de `$0.20`/`$20`. Se envió una sola orden padre MLeg `55342ded-a492-4023-b367-24cf01d88137`, client ID `polaris-auto-20260827-43d962a96c37874ee42c4d36`, `DAY`, `limit_price=0.01`, con ambas patas 1:1. No hubo fill: la orden terminó `canceled`, ambas patas terminaron `canceled`, `filled_qty=0` y no se creó orden de salida.

La verificación posterior confirmó cuenta PAPER `ACTIVE`, equity/cash `$96,914.08`, 0 posiciones, 0 órdenes abiertas y exactamente 1 orden histórica con prefijo `polaris-auto-*`, correspondiente a esta canary cancelada. No hubo impacto de P&L ni exposición residual. El documento `polaris_canary_runs/canary-paper-live-2026-08-27-1505Z` conserva la selección, la orden padre, estados terminales, cuenta y telemetría; se corrigió mediante CAS para dejar `orders_allowed=false`. El job se dejó nuevamente bloqueado con `CANARY_PREFLIGHT_ONLY=1`, ID `canary-paper-locked-after-run-2026-08-27`, imagen final `polaris-canary:aef7bbd@sha256:f6b4264810db0a4dbed275f9d0c69ecf422b8239d078aa19211c157d8b70e688`, una task, cero reintentos y timeout 600 s.

El servicio productivo no fue modificado: `polaris-bot-telegramrotate2` mantiene 100% del tráfico, PAPER y `risk.halt_new_entries=true`. La mejora posterior publicada en `aef7bbd` hace que todos los estados terminales y errores del runner queden con `orders_allowed=false`. La suite completa después de esa mejora quedó en `239 passed, 1 skipped, 2 xfailed`, con Ruff, compilación y `git diff --check` limpios.

Este resultado valida el lifecycle externo básico de una orden MLeg en PAPER —una petición padre de dos patas, timeout/cancelación, persistencia y posición plana—, pero **no demuestra rentabilidad** ni justifica ampliar capital, quitar guardarraíles, promover estrategias shadow, ejecutar otra canary o operar REAL. El token Telegram expuesto sigue pendiente de rotación adicional mediante BotFather.


## 17. Activación PAPER gradual — 27 de agosto de 2026 UTC

Tras confirmación explícita del usuario, se habilitaron únicamente entradas nuevas en PAPER con una posición abierta máxima, pérdida diaria absoluta máxima de `$20` y sin REAL. El cambio publicado en `f409468` dejó `risk.halt_new_entries=false`, `risk.max_open_positions=1` y `risk.max_daily_loss_usd=20.0`. `universo.options_reto.max_premium_net=12.0` se mantuvo sin aumento, por lo que la prima máxima por spread continúa por debajo del límite autorizado de `$20`; `direction=bull`, capas promoted y capas shadow siguen sin autoridad de órdenes. El floor, RiskManager, circuit breakers, earnings filter, validación de cotizaciones, ledger e idempotencia permanecen activos.

La imagen `polaris-bot:f409468@sha256:4f97db2f4e728095629bff136c3cac354653e9c44c1a9f5ac886c19872f76e6d` se desplegó como revisión `polaris-bot-00120-lxz`, primero sin tráfico y luego al 100% después de quedar lista. La especificación de infraestructura se conservó: PAPER, una instancia, concurrencia 1, CPU always-on, secretos existentes y polling de 60 s. La verificación inicial de tres ciclos entre 15:41:48 y 15:43:53 UTC confirmó `new_entries_halted=false`, `broker_reconciliation_halt=false`, `open_broker_orders=0`, `unmanaged_broker_legs=0`, `unmanaged_state_positions=0`, 3 escrituras Firestore, 3 `Tick OK`, 3 diagnósticos y 0 Tracebacks/CRITICAL en la revisión nueva.

En esos ciclos no hubo señales tradeables (`tradable=0`, `approved=0`, `orders=0`); esto significa que el bot ya puede entrar si aparece una señal válida, pero no debe forzar operaciones. Se observaron los warnings conocidos de SIP sin entitlement y `^VIX` inválido, ya clasificados como advertencias de feed manejadas; no se observó un fallo crítico de ejecución. El job aislado `polaris-canary-paper` permanece bloqueado en `CANARY_PREFLIGHT_ONLY=1` para impedir otra canary accidental. El token Telegram v2 continúa comprometido y requiere rotación adicional vía BotFather.


## 18. Override bajista experimental PAPER — 27 de agosto de 2026 UTC

El usuario autorizó explícitamente un override experimental para que el motor bajista opere put spreads definidos en PAPER aunque el backtest previo hubiera recomendado `min_premium_net=100` por impacto de comisiones. El override publicado en `565bc70` habilita `universo.options_bear.enabled=true`, cambia `min_premium_net` a `0.0` únicamente para PAPER y añade un tope duro de `max_premium_net=20.0`; `options_bear.max_positions=1`. El código revalida el tope antes de enviar y conserva `tp_premium_mult=1.5`, `sl_premium_mult=0.50`, expiración 14–35 DTE, put vertical MLeg 1:1, RiskManager, floor, circuit breakers, earnings, validación de quotes, ledger e idempotencia. No se habilitan puts desnudos, cortos de acciones, REAL ni capas shadow como autoridad.

La imagen `polaris-bot:565bc70@sha256:093e13ecec0e784d8e6add77bdb9f9532c1fc7402c08bd5e5b5b217face0d637` se desplegó como revisión `polaris-bot-00121-7x4` y recibió 100% del tráfico tras quedar lista; la revisión anterior permanece disponible para rollback. Entre 16:30:20 y 16:30:21 UTC se verificó arranque, 2 `Tick OK` y 2 escrituras Firestore, sin Tracebacks, CRITICAL ni Telegram 409 en la revisión nueva. Los diagnósticos registraron `new_entries_halted=false`, `broker_reconciliation_halt=false`, `orders=0`, `bear_candidates=0`, cero órdenes abiertas y cero posiciones. No hubo señal bajista CHoCH elegible en la ventana observada; por tanto, no se forzó ninguna operación.

La cuenta PAPER continuó `ACTIVE`, equity/cash `$96,914.08`, 0 posiciones y 0 órdenes abiertas. El motor queda habilitado para una señal bajista válida, pero el override está clasificado como experimental y **no** como estrategia validada: el backtest previo no demuestra rentabilidad con primas inferiores a `$100`, y las comisiones pueden consumir una operación pequeña. Mantener monitorización y rollback si aparece divergencia broker/Firestore, fill parcial, estado desconocido, error de feed crítico o incumplimiento del tope.


## 19. Tope bajista PAPER aumentado a $50 — 27 de agosto de 2026 UTC

El usuario confirmó elevar el tope de prima del motor bajista experimental de `$20` a `$50` por operación. El cambio publicado en `89f18bd` modifica únicamente `universo.options_bear.max_premium_net=50.0`; mantiene `min_premium_net=0.0` como override experimental, una sola posición total, `risk.max_daily_loss_usd=20.0`, `risk.halt_new_entries=false`, put vertical MLeg 1:1, TP/SL 1.5x/0.50x, DTE 14–35 y todos los controles de ejecución, reconciliación, ledger e idempotencia. Una operación individual puede perder hasta `$50` de prima más comisiones; el breaker diario de `$20` sigue siendo un límite de nuevas entradas y no convierte retrospectivamente una pérdida ya realizada en `$20`.

La imagen `polaris-bot:89f18bd@sha256:5dfbda8b5685e1108044bee62f81072f71263bb96602a2fe80c0407145fdc098` se desplegó como `polaris-bot-00122-5xs` y recibió 100% del tráfico; la revisión `polaris-bot-00121-7x4` queda disponible para rollback. La observación posterior confirmó `new_entries_halted=false`, `broker_reconciliation_halt=false`, 1 `Tick OK`, 1 escritura Firestore, 0 Tracebacks, 0 CRITICAL y 0 órdenes abiertas. Hubo `bear_candidates=1` en el primer ciclo, pero no se envió orden; los ciclos siguientes no tuvieron candidato. La cuenta PAPER permaneció `ACTIVE`, equity/cash `$96,914.08`, 0 posiciones y 0 órdenes abiertas. Se observó un Telegram 409 puntual durante el solapamiento de arranque/rotación; la revisión no volvió a mostrar 409 en el conteo final. Mantener vigilancia y rollback ante repetición.

El backtest EOD del 26-ago no valida este tope ni el override bajista: la evidencia usó OHLC diario, no NBBO intradía ni fills MLeg reales. El resultado agregado de 720 corridas alcistas fue media `-2.2878%`, mediana `-0.8566%` y 20.2778% positivas; el de 720 corridas bajistas fue media `-1.8067%`, mediana `-0.7950%` y 19.7222% positivas. No usar esas cifras como promesa de recuperación.


## 20. Campaña de backtesting de motores y combinaciones — 27 de agosto de 2026 UTC

Se ejecutó una campaña reproducible de investigación con tres bloques. La matriz `scripts/run_research_matrix.py` evaluó 73 configuraciones en cuatro ventanas reales con yfinance/MarketDataFeed, slippage de 2–5%, comisión de $0.65 por pata y coste de equity de 0.2%. `regime_aware` fue el mejor motor no trivial por equilibrio: retorno medio 15.6975%, mediana 12.2913%, 66.6667% de corridas positivas, drawdown medio -4.9085% y peor -9.6792%. `breakout55` promedió 9.2867% con peor drawdown -25.5463%. `put_choch`, base del motor bajista, promedió -11.7645%, con 0% de corridas positivas y profit factor medio 0.1454. `smc_daily` promedió -17.3234% y peor drawdown -59.9036%.

El holdout fijado antes de mirar resultados separó septiembre de 2025–abril de 2026 de mayo–agosto de 2026. `regime_aware` fue positivo en ambos bloques: 5.2481% en entrenamiento y 20.2038% en holdout, con drawdowns -6.8600% y -8.6515%. Una combinación `regime_hold_cash` + 30% `breakout55` también fue positiva en ambos: 22.7480% y 8.2880%, pero con drawdowns -20.5455% y -17.5179%. Estos son resultados de un simulador de opciones basado en BS/underlying y no equivalen a fills MLeg reales.

La campaña de ensembles evaluó pesos 30/50/70/100% de breakout. La mejor mediana descriptiva fue régimen hold/cash + 30% breakout55, 13.4930% en las cuatro ventanas y 3 de 4 positivas; la concentración por régimen y drawdown impiden promoción. El walk-forward adicional de relative strength dio 23.4549% medio en cuatro folds para `rs_h20_k1_r5_bull_long_only_all_c5`, pero dos folds fueron 0% por ausencia de exposición y el resultado no valida opciones; la variante long/short promedió 14.9162%.

El informe completo es `docs/backtest_campaign_2026-08-27.md` y los artefactos están en `/home/ubuntu/backtests/campaign_2026-08-27_matrix/`, `/home/ubuntu/backtests/campaign_2026-08-27_ensemble/`, `/home/ubuntu/backtests/campaign_2026-08-27_holdout/` y el CSV de relative strength. Estado: `RESEARCH_ONLY`; no promover motores, no cambiar producción y no interpretar los resultados como garantía de recuperación. La siguiente prueba propuesta es A/B PAPER en shadow de `regime_aware` contra baseline, seguida de holdout independiente con datos OPRA/NBBO y lifecycle MLeg antes de autorizar órdenes.

## 21. Motores PAPER adicionales: regime-aware, hold/cash + put_choch y relative strength — 27 de agosto de 2026 UTC
Se implementaron y publicaron en el commit `8052bde` tres rutas de señal para órdenes MLeg PAPER: `regime_aware` como fallback bull basado en régimen global y ranking RSI; `regime_hold_cash + put_choch` como ruta bear definida, activa solo con régimen bear y CHoCH por símbolo; y `relative_strength` como motor independiente que mapea líderes bull a call debit spreads y laggards bear a put debit spreads. Todas las rutas reutilizan el builder vertical, validación bid/ask, `RiskManager`, claim CAS del entry ledger, una única orden padre MLeg, reconciliación y fail-closed. No se habilitan acciones short, opciones desnudas ni REAL.

La configuración conserva una posición MLeg total máxima, `max_daily_loss_usd=20.0`, stops/TP por prima, timeout/cancelación y el tope de prima PAPER vigente: `$12` para calls bull y `$50` para puts bear. `relative_strength` puede generar órdenes, pero requiere régimen global bull/bear; en `cash` permanece sin exposición. La prioridad efectiva evita simultaneidad: los motores live existentes se evalúan primero, luego put_choch, regime-aware y relative strength; al registrar una entrada se activa el halt de reconciliación hasta confirmar el broker.

La revisión `polaris-bot-00123-8fv`, imagen `polaris-bot:8052bde@sha256:c94cda973d5b783e8718b3869b2163913e443a80ef3e572a48bb28871fbfb8ac`, recibió 100% del tráfico. La observación inicial mostró arranque sano, cuenta PAPER `ACTIVE`, equity/cash `$96,914.08`, régimen `cash` (bull 3/8, bear 0/8), `bear_candidates=0`, `regime_aware_candidates=0`, `relative_strength_candidates=0`, cero órdenes y cero posiciones. Esto es comportamiento esperado: ningún motor debe forzar una entrada cuando el régimen es lateral o no hay candidato elegible. El backtest de estos motores sigue siendo investigación y no demuestra rentabilidad; cualquier cambio de límites o promoción adicional requiere nueva validación.

La suite quedó en `244 passed, 1 skipped, 2 xfailed`, con Ruff, compilación y `git diff --check` limpios. Se añadió `tests/test_order_engine_adapters.py` y se actualizó el contrato MLeg para contar las cuatro rutas legítimas de `submit_spread`.
